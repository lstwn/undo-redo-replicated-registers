...
\title{\Large \bf Undo and Redo Support for Replicated Registers}
...
\acrshortpl{crdt} are a tool to allow for concurrent editing of a shared document without
...
\begin{tikzpicture}[node distance=0.23cm]
  ...
  \draw[casediv] ($(1a.south west)+(0,-6.9pt)$) -- ($(1e.south east)+(0,-6.9pt)$);

\end{tikzpicture}
\caption{
  Different semantics of undo and redo with two users A and B.
  The upper part (1a-1e) depicts behavior in case of two replicated registers
  and the lower part (2a-2e) shows possible outcomes for one replicated register.
...
\Cref{fig:intro-example} illustrates the two scenarios showing different outcomes
...
\begin{enumerate}
  \item Restore the color of the rectangle to black
    ((2d1) in \Cref{fig:intro-example}; \textbf{local undo})
  \item Restore the color of the rectangle to red
    ((2d2) in \Cref{fig:intro-example}; \textbf{global undo})
  \item Be not allowed at all (not shown in \Cref{fig:intro-example})
\end{enumerate}
...
\begin{enumerate}
  \item Restore the color of the rectangle to red
    ((2e1) in \Cref{fig:intro-example}; \textbf{local redo})
  \item Restore the color of the rectangle to green
    ((2e2) in \Cref{fig:intro-example}; \textbf{global redo})
\end{enumerate}
...
\Cref{fig:intro-example} and over the same sequence of operations.
...
findings from the two-register scenario in \Cref{fig:intro-example}.
...
% Assume a merge operation that merges $k$ branches into just one branch.
% Undoing that merge operation, should restore the $k$ previous siblings, too.

\section{Background on \glspl*{mvr}}\label{sec:background}
...
\begin{tikzpicture}[node distance=0.23cm]
  ...
  \draw (1c.north)+(-0.2cm,+\upperoffset) edge ["B set",optrans] ($(1b.north)+(0,+\upperoffset)$);
  \draw (1d.north)+(-0.2cm,+\upperoffset) edge ["A undo",optrans] ($(1c.north)+(0,+\upperoffset)$);
  \draw (1e.north)+(-0.2cm,+\upperoffset) edge ["B undo",optrans] ($(1d.north)+(0,+\upperoffset)$);
...
Yu et al.~\cite{Yu2015undo} deals with selective undo in the context of strings.
Selective undo is form of undo which allows a user to undo any operation,
regardless of \emph{where and when} it was generated.
In contrast, global undo allows for undoing a remote operation as well,
but it only allows for undoing in reverse chronological order.
Although the distinction between global and local undo is not very well established
in the \gls*{crdt} literature, it appears in the \gls*{ot}
literature~\cite{ressel1999reducing}.
A later work of Yu et al.~\cite{Yu2019undo} describes a generic undo mechanism
but it only applies to state-based \glspl*{crdt}.
Since Automerge is both a state-based and an operation-based \gls*{crdt},
this approach is not feasible for Automerge.
...
Although we argued against global undo behavior in \Cref{sec:semantics},
...
All artifacts of this research
(reference implementation, benchmarking code, test code and
footage of the behavior of the surveyed software, \LaTeX{} source) are publicly
available at \url{https://github.com/lstwn/undo-redo-replicated-registers}.
