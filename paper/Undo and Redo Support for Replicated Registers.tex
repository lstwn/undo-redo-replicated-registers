...
Hence, the strongest reasonable assumption is to assume that users are only
aware of their own changes.
...
% undo and redo should behave in the same way as in a single-replica setting.
...
Moreover, we require some additional state besides the operation history.
...
We track the \opidtrace{} so that the current values of the register can be
sorted by the \glspl{opid} of the undo/redo operations that restore those values,
...
The reason for differentiating between \setopkind{}s and
...
A \restopkind{} obtains its \emph{values} by traversing
the predecessor relationship but the \emph{ordering} among the siblings
produced by this traversal is determined by the \opidtrace{} using
\autoref{alg:comparison-fn}.
...
\begin{tikzpicture}[node distance=50pt]
  ...
  \begin{scope}[on background layer]
  \node[op,right of=b9] (b10) {\redop{10}{B}{5}{B}} edge [pred] (b9) edge [anchorref,] (b5);
  \end{scope}
...
the values from concurrent \setopkind{}s (yielding $[6, 1]$ for the example).
To fix this unfairness, the \opidtrace{} sorts values produced by
\emph{different heads} by their head's \glspl*{opid}
and values produced by the \emph{same head}
by their first deviation in the \opidtrace{} (resulting in $[1, 6]$).
...
To optimize the algorithm note that the predecessors of an operation and the
...
As soon we store the sorted order, we can additionally drop the
\opidtrace{} and rely on a \emph{stable} sort algorithm to include the sorted
values of a \restopkind{} at the right position with just the \opidtrace{}
up to the \restopkind{}'s \gls*{opid}.
This works because if a \restopkind{} produces multiple values,
the suffix of the \opidtrace{} starting from the \restopkind{} is responsible
for sorting the values it produces, but not responsible for
determining the sorting of the values w.r.t. other values contributed by different
heads, for which the remaining prefix of the \opidtrace{} is responsible.
...
is constant as the while loop does not add an element to the \textit{todo} list,
but just removes the \setopkind{} from the list.
...
% \Cref{fig:degenerate-op-sequence} shows such a scenario where a single replica
...
but we plan to integrate it into Automerge in the future.
...
% then undo the operation at the time of arrival at the undoing replica but
...
% the replica's own operations which are always delivered in order at each replica,
...
% one set for each replica.
% Since a replica can only undo her own operations,
% it cannot happen that concurrently the same operation is undone multiple times,
% or that the same operation is both undone and redone by different replicas.
