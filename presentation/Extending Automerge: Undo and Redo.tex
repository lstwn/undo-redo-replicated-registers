...
\begin{tikzpicture}[node distance=18pt]
  ...
  \visible<6->{
  \node [fill=white,anchor=center,draw,rounded corners] at (3.6,-1.1) {
    \visible<6->{
...
\begin{frame}{How are Replicated Registers different from, e.g., a Counter CRDT?}
  \begin{itemize}[<+->]
    \item every state update message contains the whole state
    \item what is the inverse of a set op?
    \item how to redo?
  \end{itemize}
\end{frame}
